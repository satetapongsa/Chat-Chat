...
/**
 * 🐱 CATGRAM PRO - 10S SNAP & EDGE-TO-EDGE LAYOUT
 */
...
export default function App() {
    ...
    return (
      <div className={`h-[100dvh] w-full flex items-center justify-center bg-purple-900 bg-gradient-to-br ${WALLPAPERS[0].class} p-6`}>
         <div className="w-full max-w-sm bg-white/10 backdrop-blur-3xl p-10 rounded-[4rem] shadow-2xl border border-white/10 text-center text-white">
            ...
            <button onClick={() => { if(tempName.trim()){ localStorage.setItem('catgram_user_name', tempName.trim()); setUserName(tempName.trim()); } }} className="w-full bg-white text-purple-900 font-black py-5 rounded-2xl hover:scale-105 transition-all uppercase italic">Enter Vault</button>
         </div>
      </div>
    );
  }
  ...
  return (
    <div className={`h-[100dvh] w-full flex flex-col bg-purple-950 bg-gradient-to-br ${wallpaper} overflow-hidden font-sans`}>
      <header className="px-5 py-4 flex justify-between items-center text-white z-20 backdrop-blur-md border-b border-white/5">
          <div className="flex items-center gap-3">
              ...
              <div>
                  <h2 className="font-black text-[13px] uppercase tracking-[0.2em] italic">CatGram Vault</h2>
                  {typingUsers.length > 0 && <p className="text-[10px] text-green-400 font-bold animate-pulse">Someone is typing...</p>}
              </div>
          </div>
      ...
      <main className="flex-1 overflow-y-auto px-1 sm:px-2 py-6 space-y-4 relative z-10 scroll-smooth">
          ...
          {activeMessages.map((msg, i) => {
            ...
            return (
              <motion.div key={msg.id || i} initial={{ opacity: 0, scale: 0.9, x: isMe ? 20 : -20 }} animate={{ opacity: 1, scale: 1, x: 0 }} exit={{ opacity: 0, scale: 0.5 }} className={`flex w-full flex-col mb-4 ${isMe ? 'items-end' : 'items-start'}`} onDoubleClick={() => setReplyingTo(msg)}>
                <span className={`text-[9px] font-bold text-white/30 mb-1 uppercase tracking-widest ${isMe ? 'mr-2' : 'ml-2'}`}>{msg.sender_name}</span>
                <div 
                    onClick={() => isSnap && setExpandedMedia(msg)}
                    className={`relative p-3 shadow-xl max-w-[92%] group cursor-pointer ${isMe ? 'bg-[#D1F9B3] text-[#122A09] rounded-l-[1.5rem] rounded-tr-none' : 'bg-white text-slate-900 rounded-r-[1.5rem] rounded-tl-none'}`}
                    style={{ marginInline: isMe ? '0 0.25rem' : '0.25rem 0' }}
                >
                  {msg.reply_to_name && (
                  ...
                  {isSnap ? (
                      ...
                      <p className="px-2 pb-4 pt-1 leading-relaxed font-semibold whitespace-pre-wrap break-words">{msg.content}</p>
                  )}
                  ...
                  <div className={`absolute -top-8 ${isMe ? 'right-0' : 'left-0'} opacity-0 group-hover:opacity-100 transition-opacity bg-white/95 rounded-full px-2 py-1 shadow-2xl flex gap-1.5 z-50 transform -translate-y-1`}>
      ...
      <footer className="px-2 py-4 bg-black/5 backdrop-blur-[35px] border-t border-white/5 w-full relative z-20 flex flex-col items-center">
        <AnimatePresence>
          {replyingTo && (
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="w-[calc(100%-1rem)] max-w-5xl px-4 py-2 bg-white/5 border border-white/10 rounded-t-xl flex items-center justify-between text-white text-[10px] mb-1">
        ...
        <div className="w-full max-w-5xl flex items-center gap-1 px-1">
          <div className="flex-1 flex items-center bg-white rounded-2xl px-3 py-2 shadow-2xl relative">
            ...
            <input value={inputValue} onChange={(e) => { setInputValue(e.target.value); toggleTyping(e.target.value.length > 0); }} onKeyDown={(e) => e.key === 'Enter' && handleSend(inputValue)} placeholder="Write secret..." className="flex-1 bg-transparent border-none outline-none py-2 px-2 text-slate-800 text-[15px] font-bold" />
            
            <input type="file" ref={cameraInputRef} onChange={(e)=>handleFileUpload(e, true)} accept="image/*" capture="environment" className="hidden" />
            <button onClick={() => cameraInputRef.current?.click()} className="p-1 text-slate-300 hover:text-purple-500 transition-all mr-1"><Camera size={24}/></button>
        ...
        {showEmojis && (
            <div className="absolute bottom-full left-1 right-1 bg-white border-t border-white/10 z-[60] overflow-y-auto max-h-[180px] shadow-2xl rounded-t-[2.5rem] flex justify-center">
      ...
      <AnimatePresence>
        {expandedMedia && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/95 flex flex-col items-center justify-center p-4" onClick={() => setExpandedMedia(null)}>
            <div className="w-full max-w-full relative flex flex-col items-center" onClick={e=>e.stopPropagation()}>
                {expandedMedia.type === 'image' || expandedMedia.type === 'snap' ? <img src={expandedMedia.file_url} className="w-full h-auto max-h-[85vh] rounded-[2rem] shadow-2xl border border-white/5" /> : <video src={expandedMedia.file_url} controls autoPlay className="w-full h-auto max-h-[85vh] rounded-[2rem] shadow-2xl" />}
                <div className="absolute top-6 right-6 flex flex-col items-center gap-2">
                    <div style={{ width: 48, height: 48 }}>
                        <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
                            <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="3" />
                            <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="#FFF" strokeWidth="5" strokeLinecap="round" strokeDasharray={`${Math.max(0, (expandedMedia.expires_in - (currentTime - new Date(expandedMedia.created_at).getTime())) / expandedMedia.expires_in) * 100}, 100`} />
                        </svg>
                    </div>
                </div>
                <button onClick={()=>setExpandedMedia(null)} className="mt-10 text-white/40 uppercase font-black text-[11px] tracking-[0.6em] hover:text-white transition-all">Close Secret View</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showSettings && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-2xl flex items-center justify-center p-4" onClick={()=>setShowSettings(false)}>
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="w-full max-w-sm bg-[#111] border border-white/10 rounded-[4rem] p-10 shadow-2xl" onClick={e=>e.stopPropagation()}>
                    <div className="space-y-10">
                        <div>
                            <div className="text-white/20 text-[11px] font-black uppercase tracking-[0.5em] mb-5 flex justify-between">Pulse Timer <X size={16} onClick={()=>setShowSettings(false)} className="cursor-pointer"/></div>
                            <div className="grid grid-cols-3 gap-2">
                                {LIFETIME_OPTIONS.map(opt => <button key={opt.label} onClick={() => { setCurrentLifetime(opt.value); localStorage.setItem('catgram_lifetime', String(opt.value)); }} className={`py-4 rounded-2xl text-[10px] font-bold border transition-all ${currentLifetime === opt.value ? 'bg-purple-600 border-purple-400 text-white shadow-xl shadow-purple-900/40' : 'bg-white/[0.03] border-white/5 text-white/30 hover:bg-white/[0.08]'}`}>{opt.label}</button>)}
                            </div>
                        </div>
                        <div>
                            <div className="text-white/20 text-[11px] font-black uppercase tracking-[0.5em] mb-5">Ambience</div>
                            <div className="grid grid-cols-2 gap-2 max-h-[180px] overflow-y-auto pr-1">
                                {WALLPAPERS.map(wp => <button key={wp.name} onClick={() => { setWallpaper(wp.class); localStorage.setItem('catgram_wallpaper', wp.class); }} className={`p-4 rounded-xl text-[10px] bg-gradient-to-br ${wp.class} border-2 ${wallpaper === wp.class ? 'border-white' : 'border-transparent'} text-white font-bold transition-all`}>{wp.name}</button>)}
                            </div>
                        </div>
                        <div className="pt-2">
                            <button onClick={() => { localStorage.clear(); window.location.reload(); }} className="w-full py-5 text-red-500 font-bold border border-red-500/10 rounded-[2rem] text-[11px] uppercase tracking-[0.5em]">Clear Identity</button>
                        </div>
                    </div>
                </motion.div>
            </motion.div>
